...
#mkdir ./00_ManualInit_of_WHS_Scaled
#transformix -in ./00_ManualInit_of_WHS/result.mhd -out ./00_ManualInit_of_WHS_Scaled/ -tp 00_TransformParameters_manual_initialization_Scaled.txt -threads 4

#mkdir ./00_ManualInit_of_WHS_Scaled_single
#transformix -in ./00_ManualInit_of_WHS/result.mhd -out ./00_ManualInit_of_WHS_Scaled_single/ -tp 00_TransformParameters_manual_initialization_Scaled_single.txt -threads 4
...
#mkdir ./01_Rigid_3D
#elastix -m ./00_ManualInit_of_WHS_Scaled_single/result.mhd -f ./00_ManualInit_microscope_APAdjust/result.mhd -p Parameters_Rigid_3D.txt -out ./01_Rigid_3D/ -threads 4

#-t0 Init_Transform_Rigid3D.txt

#mkdir ./01_AffineDTI
elastix -m ./00_ManualInit_of_WHS_Scaled_single/result.mhd -f ./00_ManualInit_microscope_APAdjust/result.mhd -p 01_Parameters_Affine_DTI.txt -t0 Init_Transform_Rigid3D.txt -out ./01_AffineDTI/ -threads 4
